...
def get_predicted_value(patient_symptoms):
    if len(patient_symptoms) == 1 and 'fever' in patient_symptoms:
        return diseases_list[10]
    else:
        patient_symptoms.remove('fever')
...
def index():
    if request.method == 'POST':
        ...
        desc, precautions, med, die, workout = helper(predicted_disease)
        return render_template('result.html', disease=predicted_disease, desc=desc, precautions=precautions, med=med, die=die, workout=workout)
